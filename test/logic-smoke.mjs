...
// Nothing may leave the page. chatgpt.com is React: it holds fiber references to
// the nodes it rendered, so unmounting one makes React's next removeChild or
// insertBefore throw NotFoundError in its commit phase, which tears down the
// tree and leaves a page that paints but ignores every click. Hiding is free.
assert.doesNotMatch(content, /\.replaceWith\(/, 'nodes must never be swapped out of the page — React still references them');
assert.doesNotMatch(content, /createComment\(/, 'placeholder comments mean something was detached');
assert.match(content, /data-gptdelag-old.*display:none/, 'old turns must be hidden by the stylesheet');
assert.match(content, /\[data-gptdelag-tool\]\{display:none !important\}/, 'tool embeds must be hidden by the stylesheet');
assert.match(content, /turn\.setAttribute\('data-gptdelag-old', '1'\)/, 'trimming must flag the turn, not remove it');
assert.match(content, /el\.setAttribute\('data-gptdelag-tool', '1'\)/, 'blocking must flag the embed, not remove it');
...
assert.match(mock, /this\.shadowRoot \|\| this\.attachShadow/, 'shadow fixture must survive hide and restore');
...
assert.match(content, /turn\.querySelector\(COMPOSER_SEL\)\) return/, 'hiding must re-check for the composer exactly, not from the cache');
assert.match(content, /turn\.tagName === 'MAIN'/, 'page chrome must never be hidden as a turn');
...
// Turns stay mounted now, so nothing shrinks the thread between passes the way
// detaching used to. Re-deriving every turn, or re-checking every hidden turn
// for the composer, on every pass costs ~3x of 1.4.0 on load. Both must cache.
assert.match(content, /m\.closest\('\[data-gptdelag-turn\]'\) \|\| turnOf\(m\)/,
  'getTurns must read the turn tag back rather than re-deriving it every pass');
assert.match(content, /if \(turn\.hasAttribute\('data-gptdelag-old'\)\) return;/,
  'hideOldTurn must skip turns it already hid instead of re-scanning for the composer');
